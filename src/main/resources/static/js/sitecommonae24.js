...
(function($) {
    ...
    var generateHtml = function(type, title, msg){
		...
		html += '<input name="btn_warn" class="warn_btn_yes" id="_warn_btn_yes" type="button" value="确 定" />';
		if(type == 'confirm'){
			html += '<input name="btn_warn" class="warn_btn_no" id="_warn_btn_no" type="button" value="取 消" />';
		}
	    html += '</p>';
	    html += '</div>';
	    $("body").append(html);
 ...
 */
function isMobil(s) {
    var numReg = /^((0\d{2,3})-)(\d{7,8})(-(\d{3,}))?|((\(\d{2,3}\))|(\d{3}\-))?(13|14|15|17|18|19)\d{9}$/g;
    if (!numReg.test(s)) {
        return false;
    }
    return true;
